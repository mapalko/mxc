...
impl AceRef {
    ...
    fn is_inheriting(&self) -> bool {
        (self.ace_flags & (OBJECT_INHERIT_ACE.0 | windows::Win32::Security::CONTAINER_INHERIT_ACE.0) as u8) != 0
    }
...
mod tests {
    ...
    fn check_missing_returns_false() {
        ...
        let present = check_grant(dir.path(), TEST_SID, FILE_TRAVERSE_MASK)
    ...
    fn add_then_check_then_remove() {
        ...
        let added_again =
            add_grant(path, TEST_SID, FILE_TRAVERSE_MASK).expect("idempotent add");
