...
/// On a state-aware dispatch failure, record the error in the diagnostic buffer
/// (which flushes to stderr) so it never interleaves with the stdout response
/// envelope.
...
fn run_state_aware_main(
    ...
    // On dispatch failure, record the error in the diagnostic buffer (stderr)
    // before flushing, so a failing phase still surfaces a reason on stderr.
