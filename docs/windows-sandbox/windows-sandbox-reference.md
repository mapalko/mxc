...
daemon speaks. Before streaming an `exec` to a live daemon, the backend refuses
one whose protocol version differs from the current build's: a daemon left by a
different mxc install can still authenticate (its nonce is in the record), but
its line/frame format may not match. Such a sandbox must be torn down at the
process level (see `--force-reclaim`) and re-provisioned. `stop`/`deprovision`
still use the stable single-line `STOP` command so an orphaned VM can always be
reclaimed.
